...
func _test_initial_state() -> void:
	...
	_expect(main_script._destructive_confirm_needs_tall_layout(), "enlarged Android text keeps destructive confirmation choices above the fold")
	main_script.system_font_scale = 1.0
	main_script.large_text_enabled = false
	...
	main_script.run_coordinator.start_new(101)
...
func _test_character_selection_and_scope() -> void:
	...
	_expect(coordinator.is_pristine_run(), "new run is recognized as pristine before the player makes progress")
	_expect(coordinator.select_character(1, &"hacker").ok, "second player can select hacker before route choice")
	_expect(not coordinator.is_pristine_run(), "character selection marks a run as resumable progress")
